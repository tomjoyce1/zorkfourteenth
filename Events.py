...
def coordinator_view_events(CoordinatorID): #AGAIN, USE THE VIEW FROM THE VIEW_EVENTS() FUNCTION AND JUST ADD A WHERE CONDITION TO SELECT THE RECORD WITH THE CORRECT COORDINATOR ID
    conn = sqlite3.connect('MiniEpic.db')
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM Events WHERE ClubID IN (SELECT ClubID FROM Clubs WHERE CoordinatorID = ?)", (CoordinatorID,))
    rows = cursor.fetchall()
...
def coordinator_view_events_pending(CoordinatorID):#USE THE VIEW FROM THE ABOVE FUNCTION AND JUST ADD "APPROVALSTATUS = 'PENDING'" TO THE WHERE CONDITION"
    conn = sqlite3.connect('MiniEpic.db')
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM Events WHERE ClubID IN (SELECT ClubID FROM Clubs WHERE CoordinatorID = ?) AND ApprovalStatus = 'pending'", (CoordinatorID,))
    rows = cursor.fetchall()
    result = [list(row) for row in rows]
    conn.close()
    return result
...
def admin_view_events_pending():#TRY USING THE SAME VIEW FOR THE ADMIN AND COORDINATOR FUNCTIONS BUT DISTINGUISHED BETWEEN THEM BY USING THE WHERE CONDITION
    conn = sqlite3.connect('MiniEpic.db')
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM EventRegistration WHERE ApprovalStatus = 'pending'")
    rows = cursor.fetchall()
    result = [list(row) for row in rows]
    return result
...
def verify_event_registration(user_id, EventID): 
    conn = sqlite3.connect('MiniEpic.db')
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM EventRegistration WHERE UserID = ? AND EventID = ?", (user_id, EventID))
    row = cursor.fetchone()
    conn.close()
...
def update_event(event_id, title=None, description=None, date_=None, time_=None, venue_id=None): #DO WE NEED SUCH A COMPLEX FUNCTION?
    ...
    if time_:
        update_query += " Time_=?,"
    if venue_id:
        update_query += " VenueID=?,"
    # Remove the last comma
    update_query = update_query[:-1]
    update_query += " WHERE EventID=?"
    update_values = []
    if title:
...
def reject_registration(registration_id):#ONCE A REGISRTRATION IS REJECTED, IT SHOULD BE DELETED SO USE THE CANCEL_EVENT_REGISTRATION FUNCTION INTO THIS
    conn, cursor = connect_to_database()
    cursor.execute("UPDATE EventRegistration SET ApprovalStatus='rejected' WHERE RegistrationID=?", (registration_id,))
    conn.commit()
    conn.close()
...
def create_venue(venue_name):
    conn, cursor = connect_to_database()
    cursor.execute("INSERT INTO Venues (VenueName) VALUES (?)", (venue_name,))
    conn.commit()
    conn.close()
...
def delete_venue(venue_id):
    conn, cursor = connect_to_database()
    cursor.execute("DELETE FROM Venues WHERE VenueID=?", (venue_id,))
    conn.commit()
    conn.close()
...
def update_venue(venue_id, venue_name):
    conn, cursor = connect_to_database()
    cursor.execute("UPDATE Venues SET VenueName=? WHERE VenueID=?", (venue_name, venue_id))
    conn.commit()
    conn.close()
...
def get_registered_events_for_user(user_id): #CHANGE THIS TO A VIEW AND DISPLAY ONLY RELEVANT DATA (E.G. SHOW VENUE NAME NOT ID)
    conn, cursor = connect_to_database()
    cursor.execute("SELECT * FROM EventRegistration WHERE UserID=?", (user_id,))
...
def get_venue_details(venue_id):
    conn, cursor = connect_to_database()
    cursor.execute("SELECT * FROM Venues WHERE VenueID=?", (venue_id,))
...
def get_all_venues():
    ...
    cursor.execute("SELECT * FROM Venues")
    all_venues = cursor.fetchall()
>>>>>>> ec07e2f68d48d32e23d11561dc552a7381b99251
    conn.close()